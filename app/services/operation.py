...
from app.schemas.operation import OperationCreate, OperationUpdate, Operation


async def create_operation_service(db: AsyncSession, operation_data: OperationCreate):
...
async def get_operation_by_id_service(db: AsyncSession, operation_id: int) -> Operation:
...
async def update_operation_service(db: AsyncSession, operation_id: int, operation_data: OperationUpdate) -> Operation:
...
async def delete_operation_service(db: AsyncSession, operation_id: int) -> Operation:
    existing_operation = await get_operation_by_id(db, operation_id)
    if existing_operation is None:
        raise HTTPException(status_code=404, detail=f"Operation with id {operation_id} not found.")
