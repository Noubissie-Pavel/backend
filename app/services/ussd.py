...
from app.schemas.ussdschema import UssdCreateSchema, UssdUpdateSchema, UssdSchema


async def create_ussd_service(db: AsyncSession, ussd_data: UssdCreateSchema):
...
async def get_ussd_by_id_service(db: AsyncSession, ussd_id: int) -> UssdSchema:
...
async def update_ussd_service(db: AsyncSession, ussd_id: int, ussd_data: UssdUpdateSchema) -> UssdSchema:
...
async def delete_ussd_service(db: AsyncSession, ussd_id: int) -> UssdSchema:
    existing_ussd = await get_ussd_by_id(db, ussd_id)
    if existing_ussd is None:
        raise HTTPException(status_code=404, detail=f"Ussd with id {ussd_id} not found.")
